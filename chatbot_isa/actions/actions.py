...
class ActionMensajeMasivo(Action):


    def name(self) -> Text:
        return 'action_mensaje_masivo'

    # Action entry point
    def run(self, dispatcher, tracker, domain):
        global TOKEN
        global bot
        #if algo externo is True:
            #mensaje = importar de la pagina
            #df = pd.read_excel(archivo, keep_default_na=False)
            #for usuario in df.id:
                #bot.send_message(usuario, mensaje)
...
class ActionId(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if int(usuario) not in usuarios:
            ...
            print('usuarios despues',usuarios)
        ...
        for entidad in entidades:
            ...
            for x in range(0,len(df.nombre)):
                if df.nombre[x] == valor:
                    ...
                    #str(df.nombre[x]),str(df.latitud[x]),str(df.longitud[x]) #bot.sendLocation(chat_id, latitude=df.latitud[x], longitude=longitud[x])
...
class ActionImagenCalendario(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        print('lista de archivos',list_of_files)
        latest_file = max(list_of_files, key=os.path.getctime)
        print('ultimo archivo',latest_file)
...
class ActionImagenCasillero(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        #dispatcher.utter_message(text= 'este es la imagen solicitada')
        return []
...
class ActionFuncionesISA(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        global bot
        UserID = tracker.sender_id
        print('enviando archivo pdf funciones')
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        for x in range(0,len(df.intent)):
            if df.intent[x] == intent_de_mensaje:
                ...
                #'esa ubicacion esta en mi base de datos pero noi se lo puedo enviar'#sendLocation(message.chat.id, df.latitud[x], df.longitud[x])# disable_notification = FALSE,reply_to_message_id = NULL, reply_markup = NULL)
